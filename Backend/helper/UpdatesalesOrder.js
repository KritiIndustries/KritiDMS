...
import sendIndentINDB from "./SendIndentINDB.js";
import { getLastDINumber } from "../controllers/Dealer.controller.js";
...
async function breakeSalesOrder(csrfToken, cookies, SOdata, DI) {
    ...
    try {
        ...
        if (result) {
            const data = await sendIndentINDB(payload.d, DI)
...
export const UpdateSalesOrder = async (SOdata) => {
    ...
    try {
        ...
        const { csrfToken, cookies } = await fetchCsrfToken();
        const DI = await getLastDINumber();

        // 🔹 Step 2: Loop through SOs one by one
        for (const item of SOdata) {
            try {
                const response = await breakeSalesOrder(csrfToken, cookies, item, DI);
