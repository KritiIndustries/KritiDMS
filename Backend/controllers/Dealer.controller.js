...
export const getLastDINumber = async () => {

    try {
        const getDIReq = pool.request();
        const last = await getDIReq.query(`
        SELECT TOP (1) [Vbeln]
      ,[Posnr]
      ,[Matnr]
      ,[Kwmeng]
  ...
  FROM [DMS_KNL].[dbo].[indent_Table]
    ...
    `);
        let nextDI = 1;
        if (last.recordset.length > 0 && last.recordset[0].DI) {
            const lastDI = last.recordset[0].DI;   // example: "DI-7"
            const num = parseInt(lastDI.split("-")[1]); // 7
            nextDI = num + 1; // 8
        }
        const newDI = `DI-${nextDI}`;
        console.log("Assigned DI:", newDI);
        return newDI;
    }
    catch (error) {
        console.error("❌ Error retrieving last DI number:", error);
        res.status(500).json({
            success: false,
            message: "Internal Server Error",
            error: error.message,
